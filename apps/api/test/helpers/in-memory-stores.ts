...
export class InMemoryWorkflowStore implements WorkflowStore {
  ...
  async list(filter: WorkflowListFilter) {
    ...
    const key = filter.sort === 'name' ? 'name' : filter.sort === 'createdAt' ? 'createdAt' : 'updatedAt';
    rows = [...rows].sort((a, b) => {
  ...
  async update(id: string, patch: Partial<WorkflowEntity>): Promise<WorkflowEntity | null> {
    ...
    const updated: WorkflowEntity = { ...current, ...patch, id, updatedAt: new Date().toISOString() };
...
export class InMemoryRunStore implements RunStore {
  ...
  async updateRun(
    ...
    const updated: WorkflowRunEntity = { ...current, ...patch, id, updatedAt: new Date().toISOString() };
...
export function buildWorkflowPayload(overrides: Partial<Parameters<InMemoryWorkflowStore['create']>[0]> = {}) {
  return {
    name: '测试工作流',
    description: 'desc',
    tags: ['test'],
