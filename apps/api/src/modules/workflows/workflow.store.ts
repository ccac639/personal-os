...
import { WORKFLOW_MODEL, WORKFLOW_RUN_MODEL, type WorkflowDoc, type WorkflowRunDoc } from './workflow.schema.js';
...
export class MongoWorkflowStore implements WorkflowStore {
  constructor(
    @InjectModel(WORKFLOW_MODEL) private readonly model: Model<WorkflowDoc>,
  ) {}

  async list(
    filter: WorkflowListFilter,
  ): Promise<{ items: WorkflowEntity[]; total: number }> {
...
export class MongoRunStore implements RunStore {
  constructor(
    @InjectModel(WORKFLOW_RUN_MODEL) private readonly model: Model<WorkflowRunDoc>,
  ) {}
