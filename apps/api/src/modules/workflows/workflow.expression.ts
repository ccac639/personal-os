/**
 * 变量插值与安全条件表达式求值（对齐 web 端 vars.ts / runner.ts 语义）
 *
 * - resolveTemplate：`{{a.b}}` 点路径插值，缺失变量保留原样并标记
 * - evalCondition：递归下降表达式求值，绝不执行任意代码
 *   支持：变量（点路径）、数字、字符串、布尔、null、
 *   比较（== != > < >= <=）、逻辑（&& || !）、四则（+ - * / %）、括号
 */
...
/** 渲染模板：缺失变量保留 `{{name}}` 原样并列入 missing */
...
/* ---------- 条件表达式（对齐 web runner.ts evalCondition） ---------- */
...
/**
 * 条件表达式求值（安全，不执行任意代码）。
 * 返回 { ok, result, error }；语法错误时 ok=false 且 result=false。
 */
