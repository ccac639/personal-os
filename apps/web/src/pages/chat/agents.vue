 ...
 */
import { Copy, Plus, Search, Trash2, X } from '@lucide/vue';
...
/** 表单预填（来自消息 / 灵感） */
const formPrefill = ref<{ title: string; prompt: string; source: 'message' | 'inspiration' } | null>(null);
      ...
      <div class="flex flex-col gap-3 p-3">
        ...
        >
          ...
          <input
            ...
            class="bg-transparent h-8 w-full min-w-0 text-xs outline-none"
            placeholder="搜索智能体"
            aria-label="搜索智能体"
          />
        </div>
        ...
        <div class="flex flex-col gap-0.5" role="list" aria-label="智能体类别">
          <button
            ...
            :key="c.key"
            class="hover:bg-surface-100 text-surface-800/70 hover:text-surface-900 focus-visible:ring-brand-500/40 flex items-center gap-2 rounded-md px-2 py-1.5 text-xs transition-colors focus-visible:outline-none focus-visible:ring-2"
        ...
        <label class="hover:bg-surface-100 flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-xs">
          <input
            type="checkbox"
        ...
        <div class="flex flex-col gap-1">
          ...
          <select
            ...
            :value="store.sortBy"
            aria-label="排序方式"
            @change="store.setSortBy(($event.target as HTMLSelectElement).value as typeof store.sortBy)"
          >
    ...
    <main class="flex min-w-0 flex-1 flex-col">
      ...
      <header class="flex h-11 shrink-0 items-center gap-2 border-b border-surface-100 px-3">
        <div class="min-w-0 flex-1">
          ...
          <p class="text-surface-800/40 truncate text-[10px]">共 {{ totalCount }} 个 · 显示 {{ visibleCount }} 个</p>
        </div>
        <button
      ...
      <div class="min-h-0 flex-1 overflow-y-auto p-3">
        <div v-if="store.visibleAgents.length > 0" class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          <div
          ...
          >
            ...
            <div class="absolute top-2 right-9 flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
              <button
                class="bg-surface-0/90 hover:text-brand-600 focus-visible:ring-brand-500/40 flex size-6 items-center justify-center rounded-md shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2"
              ...
              <button
                v-if="!agent.builtin"
                class="bg-surface-0/90 hover:text-red-500 focus-visible:ring-brand-500/40 flex size-6 items-center justify-center rounded-md shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2"
              ...
              <button
                v-else
                class="bg-surface-0/90 hover:text-surface-900 focus-visible:ring-brand-500/40 flex h-6 items-center justify-center rounded-md px-1.5 shadow-sm text-[10px] transition-colors focus-visible:outline-none focus-visible:ring-2"
                :aria-label="`隐藏 ${agent.name}`"
                :title="'隐藏'"
                @click="store.toggleHidden(agent.id)"
              >
                隐藏
              </button>
            </div>
