...
<template>
  ...
  >
        ...
        <button
          ...
          :class="
            project.favorite
              ...
              : 'text-surface-800/40 hover:bg-surface-100 hover:text-surface-900 opacity-0 group-hover:opacity-100'
          "
      ...
      <div class="mb-1 flex items-center justify-between text-xs">
        <span class="text-surface-800/50 flex items-center gap-1">
          ...
          <span
            ...
            class="text-surface-800/40 bg-surface-100 rounded px-1 text-[10px]"
            title="手动设置进度"
          >
            手动
          </span>
        </span>
        <span class="text-surface-900 font-medium">{{ progress }}%</span>
      </div>
    ...
    >
      ...
      <button
        type="button"
        class="bg-brand-500/10 text-brand-600 hover:bg-brand-500/20 ml-auto flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium transition-colors"
        title="为此项目快速创建任务"
        aria-label="快速创建任务"
        @click.stop="emit('quick-task', project)"
      >
        <Plus class="size-3" />
        任务
      </button>
