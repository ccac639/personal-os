...
/** 执行适配器契约（未来真实执行线替换点）；execute 接受 AbortSignal 协作式中止 */
export interface WorkflowExecutionAdapter {
  readonly name: string;
  execute(request: AdapterExecuteRequest, signal?: AbortSignal): Promise<AdapterExecuteResult>;
}
 ...
 * signal 中止：任务超时/关闭时由 engine 每节点检查，立即停止执行。
 */
  ...
  async execute(
    ...
    const result = await executeWorkflow(
      ...
      },
      {
        ...
        isAborted: signal ? () => signal.aborted : undefined,
