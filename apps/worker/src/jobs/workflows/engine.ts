...
export interface EngineDeps {
  ...
  /** 任务超时/关闭协作式中止（同步检查，与 isCancelled 互补） */
  isAborted?: () => boolean;
  /** 子流程加载（默认不支持子流程） */
...
function pushLog(ctx: RunContext, level: RunLogLevel, text: string, nodeId?: string): void {
...
async function runGraph(opts: RunGraphOptions): Promise<{
  ...
  // 任务超时/关闭中止：立即停止执行（不再产出节点结果）
  const aborted = (): boolean => {
    if (isAborted()) {
      pushLog(ctx, 'error', '任务已超时或关闭，运行中止');
      return true;
    }
    return false;
  };
  ...
  // 变量可达性警告
  const warnings = collectVariableWarnings(
    snapshot.nodes,
    scope,
  ...
  for (const nodeId of execOrder) {
    // 中止检查（任务超时/关闭）：不执行后续任何节点
    if (aborted()) return { status: 'failed', outputs, error: '任务已超时或关闭' };
    // 取消检查
...
export async function executeWorkflow(
  ...
  if (result.status === 'success') {
    pushLog(
      ctx,
      'success',
      `运行完成，共执行 ${snapshot.nodes.filter((n) => n.data?.kind !== 'annotation').length} 个节点`,
    );
