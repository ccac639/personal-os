 ...
 *
 * - 队列名与 api 端 workflow.queue.ts 保持一致
 * - 处理器职责：接收 runId → 加载运行与工作流 → 经 Adapter 执行 →
 *   结果落库（结构化日志 / 失败可恢复状态）
 * - 错误语义：
 *   · 业务失败（workflow 执行失败 / 工作流不存在）→ 落库 failed，job 正常完成（不重试）
 *   · 基础设施错误（DB / Redis 抖动）→ 落库 failed + 抛错，由 BullMQ 重试
...
export function createWorkflowRunProcessor(deps: WorkflowRunProcessorDeps): Processor {
  ...
  return async (job: Job<WorkflowRunJobData>): Promise<ProcessResult> => {
    ...
    if (run.status === 'cancelled' || run.status === 'success') {
      logger.info({ runId, status: run.status }, 'workflow-run skipped');
    ...
    if (!workflow) {
      ...
      logger.warn({ runId, workflowId: run.workflowId }, 'workflow missing, run failed');
    ...
    logger.info({ runId, workflowId: workflow.id, attempt: job.attemptsMade + 1 }, 'workflow-run started');

    // 4. 执行（执行输入 = 脱敏后的 inputSummary）
    ...
    try {
      ...
      logger.error({ runId, err: message }, 'workflow-run infra error');
      throw e;
    }
    ...
    logger.info(
      { runId, status: result.status, durationMs: result.durationMs, nodes: result.nodeResults.length },
