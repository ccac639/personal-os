...
describe('workflow engine: 拓扑与基础执行', () => {
  ...
  it('拓扑排序：annotation 节点被排除', () => {
    const { order } = topoSort(
      [
        node('a', { kind: 'trigger' }),
        node('note', { kind: 'annotation', text: '说明' }),
        node('b', { kind: 'code' }),
      ],
      [edge('e1', 'a', 'b')],
    );
...
describe('workflow engine: 子流程', () => {
  ...
  it('子流程循环引用被阻止', async () => {
    const wfA = snapshot({
      ...
      nodes: [
        node('t', { kind: 'trigger' }),
        node('sw', { kind: 'subworkflow', workflowRef: 'wf-b' }),
      ],
      edges: [edge('e1', 't', 'sw')],
    });
    const wfB = snapshot({
      ...
      nodes: [
        node('t', { kind: 'trigger' }),
        node('sw', { kind: 'subworkflow', workflowRef: 'wf-a' }),
      ],
      edges: [edge('e1', 't', 'sw')],
    });
    const result = await executeWorkflow(
  ...
  it('子流程不存在：业务失败', async () => {
    const wf = snapshot({
      nodes: [
        node('t', { kind: 'trigger' }),
        node('sw', { kind: 'subworkflow', workflowRef: 'nope' }),
      ],
      edges: [edge('e1', 't', 'sw')],
    });
    const result = await executeWorkflow(
      ...
      {},
      {},
      { sleep: noSleep, loadSubflow: async () => null },
    );
    expect(result.status).toBe('failed');
...
describe('workflow engine: 运行时限制', () => {
  ...
  it('节点数超过上限被拒绝', async () => {
    const nodes = Array.from({ length: RUN_LIMITS.MAX_NODES + 1 }, (_, i) =>
      node(`n${i}`, { kind: 'code' }),
    );
