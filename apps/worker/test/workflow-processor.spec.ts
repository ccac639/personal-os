...
function runRecord(id: string, workflowId: string, status: RunRecord['status'] = 'queued'): RunRecord {
  return {
    id,
    workflowId,
    workflowName: '测试',
...
function job(data: WorkflowRunJobData, opts: { id?: string; attemptsMade?: number } = {}): Job<WorkflowRunJobData> {
  return {
    id: opts.id ?? 'job-1',
    data,
...
describe('workflow processor', () => {
  ...
  it('业务失败：工作流不存在 → 落库 failed，不抛错（不触发重试）', async () => {
    ...
    expect(store.failed).toEqual([{ id: 'run-2', error: expect.stringContaining('工作流不存在') as unknown as string }]);
  ...
  it('基础设施错误：adapter 抛错 → 落库 failed + 抛错（触发 BullMQ 重试）', async () => {
    ...
    expect(store.failed).toEqual([{ id: 'run-3', error: expect.stringContaining('执行异常') as unknown as string }]);
  });
...
describe('workflow adapter 可替换性', () => {
  ...
  it('MongoWorkerRunStore 可实例化（schema 注册不依赖连接）', async () => {
    const { MongoWorkerRunStore, getWorkflowModel, getWorkflowRunModel } = await import(
      '../src/jobs/workflows/index.js'
    );
