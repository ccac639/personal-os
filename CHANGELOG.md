...
- 页面切换重构：移除 `<Transition mode="out-in">`（Vue 3.5 的 out-in 状态机
  与懒加载组件组合在连续/快速切换时不稳定），改为「路由守卫驱动遮罩 +
  RouterView 同步切换」——导航放行时显示系统遮罩（盖住切换瞬间），确认后
  延迟淡出；过渡窗口（~1.6s 兜底）内的新导航在 beforeEach 挂起、只放行
  最后一次，重复导航（点当前页）不激活串行化；遮罩抽离为独立组件
  `PageTransitionOverlay.vue`，状态更新不再波及路由内容子树
...
- 修复连续切换页面后内容空白：`<Transition mode="out-in">` + 懒加载组件在
  快速/连续导航时吞掉 enter 阶段，`<main>` 只剩注释节点；重构为遮罩驱动的
  同步切换（见 Changed），并新增 e2e 回归测试（连续浏览 7 页 + 快速连点）
